...
# ==========================
def run_batch_MMLIB50(data_dir="data/MMLIB50", out_dir="result/MMLIB50", timeout_s=600):
    ...
    for idx, mm in enumerate(mm_files, start=1):
        ...
        if idx % 10 == 0:
            batch = [clean_row(r) for r in results[-10:]]  # ✨ lọc lần nữa cho chắc
            with csv_file.open("a", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=FIELDS)  # dùng FIELDS ở trên
                writer.writerows(batch)

    # Ghi nốt phần còn lại (<10 cuối)
    remainder = len(results) % 10
    if remainder:
        tail = [clean_row(r) for r in results[-remainder:]]  # ✨
        with csv_file.open("a", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=FIELDS)
